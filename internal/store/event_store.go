...
func (e *EventStore) UpsertBySeq(ev *model.StageEvent) error {
	...
	const upd = `UPDATE stage_events SET source=?, actor=?, role=?, label=?, raw_timestamp=?, device=?, created_at=? WHERE id=?`
	_, err = e.s.db.Exec(upd, string(ev.Source), string(ev.Actor), string(ev.Role),
		ev.Label, ev.RawTimestamp, ev.Device, ev.CreatedAt.Format(time.RFC3339), existingID)
	return err
}
...
func (e *EventStore) SetCorrectedAt(id string, correctedAt int64) error {
	...
	res, err := e.s.db.Exec(q, correctedAt, id)
	if err != nil {
		return err
	}
