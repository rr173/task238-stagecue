...
func (p *PackageStore) SetState(id string, st model.State, releasedAt time.Time, supersededBy string) error {
	const q = `UPDATE cue_packages SET state=?, released_at=?, superseded_by=? WHERE id=? OR superseded_by=?`
	res, err := p.s.db.Exec(q, string(st), releasedAt.Format(time.RFC3339), supersededBy, id, supersededBy)
	if err != nil {
		return err
	}
