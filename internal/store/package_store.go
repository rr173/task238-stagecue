...
func (p *PackageStore) UpdateDigest(id, digest string) error {
	_, err := p.s.db.Exec(`UPDATE cue_packages SET snapshot_digest=? WHERE id=? AND state=?`, digest, id, string(model.StatePublished))
	return err
}
