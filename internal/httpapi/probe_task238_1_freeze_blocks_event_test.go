...
func TestBug01_FrozenRehearsalRejectsEventAfterFreeze(t *testing.T) {
	...
	if status := postBug01(t, srv.URL+"/api/rehearsals", map[string]any{"name": "freeze", "show": "show"}, &rep); status != http.StatusCreated {
		t.Fatalf("create status=%d", status)
	}
	if status := postBug01(t, srv.URL+"/api/rehearsals/"+rep.ID+"/freeze", map[string]any{}, &map[string]any{}); status != http.StatusOK {
		t.Fatalf("freeze status=%d", status)
	}
	...
	if status != http.StatusConflict {
		t.Fatalf("frozen event status=%d, want %d", status, http.StatusConflict)
	}
}
